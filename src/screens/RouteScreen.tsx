...
function fmtArrival(h: number): string {
...
const ROW_BG: Record<string, string> = {
  lac: '#F0FDF4', biv: '#FFFBEB', star: '#EFF6FF', jstart: '#EFF6FF',
};
    ...
    <TouchableOpacity style={s.card} onPress={onPress} activeOpacity={0.8}>
      <View style={s.cardHeader}>
        <View style={s.numBadge}>
          <Text style={s.numText}>{etape.numero}</Text>
        </View>
        <View style={s.cardInfo}>
          <Text style={s.cardNom}>{etape.nom}</Text>
          <Text style={s.cardSub}>{etape.depart} → {etape.arrivee}</Text>
          <Text style={s.cardMeta}>{etape.distance} km · {etape.dureeEstimee}h</Text>
        </View>
        <View style={[s.diffBadge, { backgroundColor: DIFF_COLOR[etape.difficulte] }]}>
          <Text style={s.diffText}>{DIFF_LABEL[etape.difficulte]}</Text>
        </View>
      </View>
    ...
    <Modal visible animationType="slide" presentationStyle="pageSheet">
      <SafeAreaView style={s.modal}>
        <View style={s.modalHeader}>
          <Text style={s.modalTitle}>Étape {etape.numero} · {etape.nom}</Text>
          <TouchableOpacity onPress={onClose} style={s.closeBtn}>
            <Text style={s.closeBtnText}>✕</Text>
          </TouchableOpacity>
        </View>
        <ScrollView contentContainerStyle={s.modalContent}>
          <View style={[s.diffBadgeLarge, { backgroundColor: DIFF_COLOR[etape.difficulte] }]}>
            <Text style={s.diffTextLarge}>{DIFF_LABEL[etape.difficulte]}</Text>
          </View>
          ...
          <View style={s.routeBox}>
            <View style={s.routeRow}>
              ...
              <Text style={s.routeText}>{etape.depart}</Text>
            </View>
            <View style={s.routeLine} />
            <View style={s.routeRow}>
              <View style={[s.dot, { backgroundColor: '#E63946' }]} />
              <Text style={s.routeText}>{etape.arrivee}</Text>
            </View>
          </View>
      ...
      {waypoints.length > 0 ? (
        ...
        <View style={gpx.emptyBox}>
          ...
          <Text style={gpx.emptySub}>
            Ce GPX contient uniquement le tracé.{'\n'}Ajoutez des waypoints dans votre application GPS.
          </Text>
  ...
  return (
    <View style={it.daySection}>
      ...
      {day.waypoints.map((wpt, i) => (
        <ItWayptRow key={i} wpt={wpt} isFirst={i === 0} isLast={i === day.waypoints.length - 1} />
      ))}
    </View>
  );
}
...
function useHtmlImporter(onContent: (content: string) => void) {
  ...
  useEffect(() => {
    ...
    const handler = (e: Event) => {
      ...
      reader.onload = (ev) => {
        ...
        if (content) cbRef.current(content);
      };
  ...
  return () => {
    if (Platform.OS === 'web') {
      ...
      Alert.alert('Web uniquement', "L'import de fichiers HTML est disponible uniquement depuis l'application web.");
    }
  };
}
  ...
  return (
    <SafeAreaView style={s.container}>
      ...
      <View style={s.header}>
        <View style={s.headerRow}>
          <View style={{ flex: 1 }}>
            ...
            {trekPlan && (
              <Text style={s.trekNote}>📅 {trekPlan.nom}</Text>
            )}
          </View>
      ...
      {(hasGpxTab || hasHtmlTab) && (
        <View style={s.tabRow}>
          <TouchableOpacity
            style={[s.tab, activeTab === 'gr10' && s.tabActiveGr10]}
            onPress={() => setActiveTab('gr10')}
          >
            <Text style={[s.tabTxt, activeTab === 'gr10' && s.tabTxtGr10]}>GR10</Text>
          </TouchableOpacity>
          ...
          {activeTab === 'html' && (
            <TouchableOpacity style={s.tabClear} onPress={() => setItineraire(null)}>
              <Text style={s.tabClearTxt}>✕</Text>
            </TouchableOpacity>
          )}
        </View>
      )}
...
const s = StyleSheet.create({
  ...
  trekNote:        { color: '#E9C46A', fontSize: 12, marginTop: 4 },
  importBtn:       { backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 6, alignItems: 'center', gap: 2 },
  importBtnIcon:   { fontSize: 18 },
  importBtnLabel:  { fontSize: 10, color: '#A8DADC', fontWeight: '600' },
  tabRow:          { flexDirection: 'row', backgroundColor: '#fff', borderBottomWidth: 1, borderBottomColor: '#eee', paddingHorizontal: 12, paddingTop: 8, paddingBottom: 0, gap: 4, alignItems: 'flex-end' },
  tab:             { paddingHorizontal: 12, paddingVertical: 8, borderBottomWidth: 3, borderBottomColor: 'transparent', borderRadius: 4, maxWidth: 160 },
  tabActiveGr10:   { borderBottomColor: '#264653', backgroundColor: '#F1FAEE' },
  ...
  cardSub:         { fontSize: 12, color: '#555', marginTop: 1 },
  ...
  modalTitle:      { color: '#fff', fontSize: 15, fontWeight: '700', flex: 1, marginRight: 8 },
  closeBtn:        { padding: 4 },
  ...
  routeBox:        { backgroundColor: '#fff', borderRadius: 10, padding: 14, shadowColor: '#000', shadowOpacity: 0.04, shadowRadius: 4, elevation: 1 },
  routeRow:        { flexDirection: 'row', alignItems: 'center', gap: 10 },
  dot:             { width: 10, height: 10, borderRadius: 5 },
  routeLine:       { width: 2, height: 20, backgroundColor: '#ddd', marginLeft: 4, marginVertical: 2 },
  routeText:       { fontSize: 15, fontWeight: '600', color: '#264653' },
